...
- Next.js 16.2 (App Router) · TypeScript 5 · React 19.2 · Tailwind CSS 4 · Node runtime
- Drizzle ORM 0.45 + drizzle-kit 0.31 · DB: local SQLite via `@libsql/client` 0.17 (`drizzle-orm/libsql`), one-line swap to Neon Postgres
- Auth: BetterAuth 1.6 (email + password)

**Things training gets wrong here — the corrections:**
- **BetterAuth and Drizzle are NEWER than training — verify against their docs, don't revert from memory.** BetterAuth docs: better-auth.com (Next.js + Drizzle adapter). Drizzle docs: orm.drizzle.team (libsql + Neon).
- BetterAuth server config lives in `lib/auth.ts` using `drizzleAdapter(db, { provider: "sqlite", schema })`; the Next route handler is `app/api/auth/[...all]/route.ts` via `toNextJsHandler(auth)`. Do not rewrite these to an older shape.
- The auth tables (`user`/`session`/`account`/`verification`) in `db/schema.ts` are owned by BetterAuth — property names must match its expectations exactly. Don't rename them.
- DB is a one-line swap: change `DATABASE_URL`, the `provider` in `lib/auth.ts`, and the `dialect` in `drizzle.config.ts`. Schema stays identical. Don't fork the schema per environment.
